 ...
 * APU : minigb_apu
 *
 * Arduino IDE: config.h でハードウェアを設定してください
 * PlatformIO : platformio.ini の build_flags で設定します
 */
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (M5.BtnC.wasPressed()) {
        if (selected < count - 1) {
          selected++;
          if (selected >= scroll + ITEMS) scroll = selected - ITEMS + 1;
        }
        moved = true;
      }
